...
const user4 = db.create('users', { name: 'Rose', age: 27 });
const user5 = db.create('users', { name: 'Jane', age: 26 });
...
// Find all by query
const allAlices = db.findAllByQuery('users', { name: 'Alice'});
console.log('All users named Alice:', allAlices);

// Find all by query also find with Multiple Fields (AND clause)
const allAlicesWithAge = db.findAllByQuery('users', { name: 'Alice',  age : 25});
